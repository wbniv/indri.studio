...
- [ ] **2026-06-27** Re-fix `biohack.net/blossom` HUD clip — the player bundle is vendored per-site, and the `space-invaders` commit (`c20b62e`) re-synced `public/play/app.js` back to `yoff=8`, clobbering the fix (`3f9c66e`/`v1.0.74`); live is clipped again. Re-sync `public/play/` from the fixed `bsnes-jg-wasm` bundle (or re-apply `yoff=0`), then `task bump`. Confirm `deploy-bundle.sh` regenerates the gitignored `dist-bundle/` from `web/app.js` so future syncs carry the fix — [plan §Regression](docs/plans/2026-06-27-emulator-hud-overscan-crop-fix.md#regression--biohack-re-synced-back-to-yoff8)
...
- [x] **2026-06-27** Fixed `/blossom` HUD clipping — `present()` cropped raw rows `[8,232)` (`yoff=8` "skip overscan"), but the active picture sits flush at the buffer top; gate refs use `[0,224)`. Set `yoff=0` in indri (`7b82611`, live) + `bsnes-jg-wasm` source (`aaacbae`); also centred the indri player (`82274c3`). Verified live at 125% zoom (20 px margin). _biohack shipped (`v1.0.74`) then regressed — see active item below_ — [plan](docs/plans/2026-06-27-emulator-hud-overscan-crop-fix.md)
