...
> - **Recommendation #8 (inline critical CSS)** plan written in commit `1115347` (`docs/plans/2026-05-13-inline-critical-css.md`) but **the `astro.config.mjs` change has not landed** — `inlineStylesheets` is still default, and prod still emits an external `/_astro/Base.<hash>.css`. Treat #8 as **open**, not shipped.
...
| #8 | Inline critical CSS        | _plan only_  | not measurable — `astro.config.mjs` change never wired in                  | **open** |
