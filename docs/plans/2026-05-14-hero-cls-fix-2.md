...
- The Material Symbols `font-size: 40px/48px !important` rule lives in `global.css` lines 415-443 (per investigation report). The new `width`/`height` rules go alongside it.
...
2. **Visual: hero headline doesn't reflow on hard reload.** Open `/` in an incognito window with DevTools → Network → "Slow 4G". Hard reload. Watch the 120 px headline during first paint — it should render once at its final position, no horizontal/vertical pop. PASS criterion: zero visible jump.
...
- **Wiring Rec #8 (inline critical CSS).** Different audit recommendation, separate plan at `docs/plans/2026-05-13-inline-critical-css.md`.
